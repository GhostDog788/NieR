...
By the end of this chapter, you should be able to follow a publication through `selac`, distinguish its validation and compilation stages,
and explain why the result is an ordinary native program.
You should already understand the producer/artifact/consumer distinction, basic Sela operations, and translation units. You do not need to understand the native ABI reconstruction algorithms yet; later chapters open those boxes.

The starting point is deliberately **after publication**. The input is a standalone `.sela` archive, not a C source file.
...
```sh
build/prealpha/selac hello.sela -o hello
```

The executable named `selac` is a driver around the public Sela core, artifact handling, and existing native tools.
...
archive
  ...
  -> validate/specialize Sela for that target
...
The driver first calls `readPackage` and `validatePackage`. An archive being readable does not establish that it is a valid Sela publication.
...
The artifact kind matters too. A relocatable Sela object is an intermediate publication input: the normal stock-Clang publication link must combine it before `selac` emits a final native product.
...
`readModule` reads bounded bytecode, performs MLIR structural verification, and invokes Sela's closed schema and semantic validation.
Unknown operations or attributes do not become optional merely because MLIR can print them.
In particular, opening a dump with `mlir-opt --allow-unregistered-dialect` is a viewing convenience, not Sela validation.

## What specialization actually fills in

Suppose a common function contains a value of type `!sela.word`.
On the current wide target, lowering creates an LLVM `i64`; on the narrow target it creates `i32`.
A fixed Sela `i32` stays 32 bits. A symbolic `pointer_bytes` constant becomes 8 or 4, whereas a literal integer 8 remains 8.
...
The core's `Lowerer` creates an LLVM module with the selected triple and data layout. It resolves declarations and globals, creates native blocks, maps Sela values to LLVM values, emits instructions, and verifies the resulting module.
Sela block arguments become LLVM PHIs where needed.
...
The destination never asks which source file extension produced those functions. A direct Sela producer can create equivalent public operations.
...
It does not open its `.sela` archive, run `selac`, or invoke LLVM when the program starts.
There is no Sela bytecode interpreter or publication-specific JIT in this execution path.

Ordinary native dependencies can still be required. A dynamically linked Hello needs its selected libc and loader.
“No Sela runtime” does not mean “no libraries,” and language-blind compilation does not prohibit a future language from depending on a native garbage collector or other runtime code.
...
source sdk/env.sh
set -euo pipefail
guide13_work=$(mktemp -d "${TMPDIR:-/tmp}/sela-guide13-XXXXXX")
clang --config="$PWD/build/prealpha/sela.cfg" -O2 \
  examples/hello/hello/main.c examples/hello/hello/hello.c -o "$guide13_work/hello.sela"
build/prealpha/selac inspect "$guide13_work/hello.sela"
build/prealpha/selac lower "$guide13_work/hello.sela" \
...
build/prealpha/selac "$guide13_work/hello.sela" \
  --sdk "$SELA_SDK_ROOT" -o "$guide13_work/hello"
...
> [!faq]- Does `selac lower --target i686` establish full i686 deployment support?
>
> No. An x86-64 `selac` rejects that foreign target. An i686 `selac` can expose its own diagnostic specialization, but full deployment still requires real-device compiler/output and corpus qualification.

> [!faq]- Can the generated executable need libc but not need Sela?
>
...
then follow `lowerCompilationUnit` (`src/ir/CompilationUnits.cpp`) into the core interfaces (`include/sela/IR/Compiler.h`) and lowering implementation (`src/ir/NativeLowering.cpp`).
