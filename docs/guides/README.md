# Learning Sela: from C programmer to compiler maintainer

Sela is a compiler project, a portable program representation, and a native publication workflow.
Understanding all three at once is difficult.
This series builds the ideas in dependency order: first understand what Sela Code means, then use it, then follow its implementation, and finally examine the algorithms and evidence a maintainer must preserve.
...
Start with [Use Sela with your C project](02-toolchain-users/using-sela-with-your-c-project.md).
It is a standalone, command-oriented guide for existing Make and CMake applications: check the native build, configure the external integration, produce a `.sela` artifact, and compile and run it with `selac`.
...
| --- | --- | --- |
| Foundations | 01–06 | Explain Sela's contract and read small Sela programs. |
...
- [01-foundations](01-foundations/README.md) — chapters 01–06: Sela Code and compiler concepts.
...
## Part I — Understanding Sela Code
...
1. [Meet Sela Code](01-foundations/01-meet-sela-code.md) — the problem, the public boundary, and the three distinct activities of publication, compilation, and execution.
...
3. [Reading a Sela Program](01-foundations/03-reading-a-sela-program.md) — read actual operations, values, types, attributes, and a complete small function.
...
6. [From Sela Code to a Publication Artifact](01-foundations/06-publication-artifacts.md) — bytecode, manifests, modules, dependencies, and compilation-unit plans.
...
11. [Implementing the Sela Contract](03-contributors/11-implementing-the-sela-contract.md) — dialect, validation, serialization, and an independent producer.
...
13. [Following `selac`](03-contributors/13-following-selac.md) — validation, specialization, LLVM, native linking, and the consumer boundary.
...
24. [Maintaining and Evolving Sela](04-maintainers/24-maintaining-and-evolving-sela.md) — evidence, qualification, format changes, extension obligations, and the separate future security platform.
...
Correctness checks are not SESela (SES), the separate security platform planned above Sela and not implemented yet.
...
Start with [01 — Meet Sela Code](01-foundations/01-meet-sela-code.md).
