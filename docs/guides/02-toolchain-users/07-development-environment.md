...
Sela makes those concerns easier to see because publication and native compilation happen at different times.

The **development host** executes CMake, Ninja, Clang, and the Sela tools. These programs have their own native library dependencies.
...
Building Sela still uses some host development facilities, including C++ standard-library headers and startup files.
...
Sourcing the file changes the current shell. It places SDK tools on `PATH`, sets `SELA_SDK_ROOT`, and exports paths used by CMake and dynamic library loading.
...
Finding the expected `cmake` executable does not prove it can load its libraries. A startup error about `librhash.so.0` is an environment problem, not evidence that Sela's IR is invalid.
...
```bash
source sdk/env.sh
guide_work=$(mktemp -d "${TMPDIR:-/tmp}/sela-guide07-XXXXXX")
...
If you later choose to build this configuration, its generated `sela.cfg` and executables belong together.
...
The [build instructions](../../reference/building-sela.md) and preset describe creating it.
Each later lab uses fresh application outputs, so reading the course does not require repeatedly rebuilding Sela itself.
...
Then check the generated configuration and compiler versions. Only after those checks should you debug a failing Sela transformation.
