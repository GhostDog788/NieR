...
[Guide series](../README.md) · [Practical C-project guide](../02-toolchain-users/using-sela-with-your-c-project.md)
...
In Sela this is a bounded claim over a declared target domain, not automatic support for every CPU.
See [chapter 05](../01-foundations/05-architecture-neutral-meaning.md).

**Archive.** A container of named members.
A `.sela` publication is a tar archive; a native `.a` static library uses a different archive format and participates in native link selection.
See [chapter 06](../01-foundations/06-publication-artifacts.md) and [chapter 21](../04-maintainers/21-build-and-link-semantics.md).

**Artifact.** The standalone publication file passed from a producer to `selac`.
It contains a manifest and Sela bytecode, not the producer's private source tree or LLVM captures.
...
Sela uses LLVM's native code-generation machinery after its own specialization and lowering.
...
Sela uses MLIR bytecode for module payloads; it is not native CPU machine code and does not imply a bytecode interpreter at execution time.
See [chapter 06](../01-foundations/06-publication-artifacts.md).

**Capture.** Private producer-side evidence from a native compilation, including LLVM and associated build information.
Captures help the C producer recover Sela Code; they are not the public program given to the device.
...
**Clang.** The existing C-family frontend and compiler driver used by Sela's C producer.
The publication entry point is actual stock Clang configured with the Sela integration, not a renamed replacement compiler.
...
**Consumer.** The program or library that accepts the public Sela contract.
The standalone compiler `selac` is a consumer; it does not need the original source-language frontend.
See [chapter 13](../03-contributors/13-following-selac.md).

**Contract.** The explicit rules for what a representation means and which inputs are valid.
Sela's contract includes types, operations, attributes, target semantics, and artifact rules—not just a file suffix.
...
The custom Sela dialect is part of Sela's implementation; MLIR itself does not supply Sela's portability contract.
See [chapter 11](../03-contributors/11-implementing-the-sela-contract.md).
...
A Sela-built program can still depend on native DSOs.
...
A `.sela` artifact is not an ELF executable, even when the producer was asked to publish an executable application.
...
Sela has a corresponding `sela.gep` operation.
See [chapter 18](../04-maintainers/18-native-storage.md).
...
Sela's destination compilation is AOT even though it happens on the device: the resulting program runs as native code without a Sela execution engine.
See [chapter 01](../01-foundations/01-meet-sela-code.md).

**Linker.** The tool that resolves symbols and combines selected code and data into an output.
Sela publication linking and the later native LLD link are separate steps.
See [chapter 21](../04-maintainers/21-build-and-link-semantics.md).
...
Sela lowering selects native meanings and produces LLVM IR before optimization, machine-code generation, and native linking.
See [chapter 13](../03-contributors/13-following-selac.md).
...
Packaging multiple Sela modules together does not by itself authorize whole-program optimization or erase native compilation boundaries.
See [chapter 21](../04-maintainers/21-build-and-link-semantics.md).
...
Sela uses a custom MLIR dialect and its bytecode infrastructure; MLIR is not a source language or an execution runtime.
See [chapter 02](../01-foundations/02-compiler-foundations.md).
...
**Sela Code / `selac`.** Sela Code is the public architecture-neutral program representation.
`selac` is the separate language-blind compiler consuming it.
...
**Pointer / `!sela.ptr`.** A value used to refer to data storage or callable code.
Sela keeps the pointer type distinct from its native-width integer type; equal native widths do not make pointers and integers interchangeable in every operation.
See [chapter 05](../01-foundations/05-architecture-neutral-meaning.md).
...
See [chapter 24](../04-maintainers/24-maintaining-and-evolving-sela.md).

**Producer.** Software that emits valid public Sela Code and artifacts.
...
A Sela function's body is a region; region semantics depend on the owning operation.
See [chapter 03](../01-foundations/03-reading-a-sela-program.md).
...
Excluding source and private metadata is not encryption, nor proof that Sela artifacts resist analysis as well as native binaries.
See [chapter 01](../01-foundations/01-meet-sela-code.md).
...
**SESela / SES.** The planned security platform above the standalone Sela toolchain.
The `SE` prefix means “Security Enhanced,” as in SELinux; SES is the short name.
SESela is not implemented yet. Its signing and executable-memory enforcement policies are separate from ordinary Sela publication, compilation, and artifact validation.
Using Sela does not require SESela.
See [chapter 01](../01-foundations/01-meet-sela-code.md) and [chapter 24](../04-maintainers/24-maintaining-and-evolving-sela.md).
...
A Sela verifier is not a general vulnerability detector, trusted-publisher check, or substitute for the separate security enforcement platform.
See [chapter 22](../04-maintainers/22-artifact-validation-and-robustness.md).
...
Sela's ordinary toolchain flow is independent of the separately planned SESela security enforcement work.
See [chapter 24](../04-maintainers/24-maintaining-and-evolving-sela.md).

**Word / `!sela.word`.** Sela's native-width integer type, specialized according to the selected target's pointer width.
A fixed `i32`, a native word, and a pointer express different meanings even when widths coincide.
See [chapter 05](../01-foundations/05-architecture-neutral-meaning.md).

Return to the [course index](../README.md), or start with the [practical C-project guide](../02-toolchain-users/using-sela-with-your-c-project.md).
