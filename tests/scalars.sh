...
"${NIER_REFERENCE_LOWER:-$(dirname -- "$nierc")/nier_reference_lower}" lower "$test_work/scalars.nier" --target x86_64 --output-dir "$test_work/lowered"
rg -q 'noinline' "$test_work/lowered/0.ll"
"${NIER_REFERENCE_LOWER:-$(dirname -- "$nierc")/nier_reference_lower}" lower "$test_work/scalars.nier" --target i686 --output-dir "$test_work/lowered32"
