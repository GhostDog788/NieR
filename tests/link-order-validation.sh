...
work=$(mktemp -d -t sela-link-order-validation-XXXXXX)
inputs=()
...
"$selac" "$work/valid.sela" -o "$work/program"
...
cp "$work/valid.sela" "$work/retained.sela"
retained_identity=$(stat -c '%d:%i:%s:%y:%z' "$work/retained.sela")
failures=0
reject() {
  local diagnostic=$1
  shift
  if "$linker" "${inputs[@]}" "$@" -o "$work/retained.sela" \
      >"$work/rejection.log" 2>&1; then
    printf 'ERROR: malformed native-unit order was accepted: %s\n' "$*" >&2
    exit 1
  fi
  rg -q "$diagnostic" "$work/rejection.log"
  cmp "$work/retained.sela" "$work/valid.sela"
  test "$(stat -c '%d:%i:%s:%y:%z' "$work/retained.sela")" = "$retained_identity"
  failures=$((failures + 1))
}
