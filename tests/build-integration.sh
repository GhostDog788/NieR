  ...
  for system in make cmake; do
    ...
    if test "$system" = make; then
      make -f "$test_root/sdk/share/sela/Sela.mk" \
        ...
        SELA_NATIVE_OUTPUT=hello SELA_TARGETS=hello SELA_ARTIFACT="$artifact"
    else
...
make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
  ...
  SELA_TARGETS=libselafixture.so SELA_ARTIFACT="$test_work/shared.sela"
...
make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
  ...
  SELA_TARGETS=probe SELA_ARTIFACT="$test_work/shared-app.sela"
...
make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
  ...
  SELA_TARGETS=empty.a SELA_ARTIFACT="$test_work/static-empty.sela"
...
for system in make cmake; do
  ...
  if test "$system" = make; then
    make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
      ...
      SELA_NATIVE_OUTPUT=libduplicate.a SELA_TARGETS=libduplicate.a SELA_ARTIFACT="$artifact"
    member=member.o
  else
    "$SELA_SDK_ROOT/host/usr/bin/cmake" \
...
for selected in ordinary group whole; do
  make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
    ...
    SELA_TARGETS="$selected" SELA_ARTIFACT="$test_work/duplicates-$selected.sela"
...
if make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
    ...
    SELA_TARGETS=whole-native-error SELA_ARTIFACT="$test_work/whole-native-error.sela" \
...
for selected in archive thin whole group renamed hash-override; do
  make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
    ...
    SELA_TARGETS="$selected" SELA_ARTIFACT="$test_work/$selected.sela"
...
make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
  ...
  SELA_TARGETS=hello SELA_ARTIFACT="$test_work/source-selection.sela"
...
make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
  ...
  SELA_TARGETS=hello SELA_ARTIFACT="$test_work/build-make.sela"
...
if "$build_tool" --system make --source "$test_root/tests/fixtures/build" \
  --output hello --target hello --artifact "$test_work/build-make.sela" \
...
if make -f "$test_root/sdk/share/sela/Sela.mk" SELA_BUILD_TOOL="$build_tool" \
  SELA_SOURCE_DIR="$test_root/tests/fixtures/build" SELA_NATIVE_OUTPUT=hello \
  SELA_TARGETS=sela-intentionally-missing-target SELA_ARTIFACT="$test_work/build-make.sela"; then
