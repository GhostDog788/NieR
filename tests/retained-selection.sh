...
build_tool="$(dirname -- "$config")/nier-build"
test_work=$(mktemp -d "${TMPDIR:-/tmp}/nier-retained-selection-XXXXXX")
...
"$build_tool" --system make --source "$project/tests/fixtures/link-order" \
  --target libfirst.a --output libfirst.a --artifact "$test_work/library.nier" \
...
"$replayer" "$config" "$sdk" "$retained" source/libfirst.a "$test_work/library.nier"
...
if "$replayer" "$config" "$sdk" "$retained" source/libfirst.a "$test_work/library.nier" \
