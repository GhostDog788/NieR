...
work=$(mktemp -d -t sela-linking-XXXXXX)
trap 'rm -r -- "$work"' EXIT
...
"$llvm/clang" --config="$config" -O2 -shared "$fixture/api.c" \
  ...
  -o "$work/library.sela"
"$selac" "$work/library.sela" --sdk "$sdk" -o "$work/libraries/libexample.so.1"
...
# destination linking. This fixture is a conventional native DSO, not Sela IR.
...
"$llvm/clang" --config="$config" -O2 "$fixture/main.c" \
  ...
  -o "$work/main.sela"
"$selac" "$work/main.sela" --sdk "$sdk" --library-dir "$work/libraries" -o "$work/main"
...
if "$selac" "$work/main.sela" --sdk "$sdk" -o "$work/main" > "$work/missing.log" 2>&1; then
...
"$llvm/clang" --config="$config" -O2 "$fixture/main.c" \
  ...
  -o "$work/gnu.sela"
"$selac" "$work/gnu.sela" --sdk "$sdk" --library-dir "$work/libraries" -o "$work/gnu"
...
for loader in '-Wl,--dynamic-linker,/custom/loader' '-Wl,--dynamic-linker=/custom/loader'; do
  if "$llvm/clang" --config="$config" -O2 "$fixture/dynamic.c" "$loader" \
      -o "$work/custom-loader.sela" > "$work/loader.log" 2>&1; then
    echo 'Custom interpreter was silently dropped' >&2
    exit 1
  fi
  grep -q 'unqualified native dynamic interpreter' "$work/loader.log"
done
"$llvm/clang" --config="$config" -O2 "$fixture/dynamic.c" -ldl -o "$work/dynamic.sela"
"$selac" "$work/dynamic.sela" --sdk "$sdk" -o "$work/dynamic"
