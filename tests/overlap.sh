...
work=$(mktemp -d -t nier-overlap-XXXXXX)
trap 'rm -r -- "$work"' EXIT
for level in O0 O2; do
  ...
  "$llvm/clang" --config="$config" -"$level" "$project/tests/fixtures/overlap.c" -o "$work/program.nier"
  "$nierc" "$work/program.nier" --sdk "$sdk" -o "$work/program"
