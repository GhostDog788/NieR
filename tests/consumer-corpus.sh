...
record_output() {
    ...
    printf 'NIER_CORPUS_NATIVE_OUTPUT_SHA256 '
    sha256sum "$1"
}
(
    cd "$fixtures"
...
while IFS= read -r project; do
    ...
    case "$project" in
        cjson-static|cjson-shared)
            ...
            if [ "$project" = cjson-shared ]; then
                "$compiler" "$pack/artifacts/library.nier" -o "$destination/libcjson.so.1"
                ...
                "$compiler" "$pack/artifacts/library.nier" -o "$destination/libcjson.a"
            ...
            while IFS= read -r program; do
                ...
                "$compiler" "$pack/artifacts/$label.nier" --library-dir "$destination" -o "$destination/$program"
                record_output "$destination/$program"
                programs=$((programs + 1))
            done < "$pack/programs.list"
            test "$programs" -eq 20
            for directory in . tests fuzzing; do
                sed "s|@NIER_CORPUS_DESTINATION@|$destination|g" \
            ...
            ;;
        zlib)
            "$compiler" "$pack/artifacts/static-library.nier" -o "$destination/libz.a"
            sha256sum "$destination/libz.a"
            archive_tool t "$destination/libz.a" > "$destination/archive-members.txt"
            cmp "$pack/archive-members.txt" "$destination/archive-members.txt"
            "$compiler" "$pack/artifacts/library.nier" -o "$destination/libz.so.1"
            ...
            for program in example minigzip examplesh minigzipsh example64 minigzip64; do
                "$compiler" "$pack/artifacts/$program.nier" --library-dir "$destination" -o "$destination/$program"
                record_output "$destination/$program"
            done
    ...
    printf 'NIER_CORPUS_PROJECT_PASS %s\n' "$project"
...
printf 'NIER_CONSUMER_CORPUS_PASS target=i686 artifacts=%s\n' "$count"
